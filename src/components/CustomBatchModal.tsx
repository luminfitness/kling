...
interface CustomBatchModalProps {
  isOpen: boolean;
...
export default function CustomBatchModal({
  isOpen,
  templates,
  onClose,
  onComplete,
}: CustomBatchModalProps) {
  // Phase: 'edit-prompt' or 'processing'
  const [phase, setPhase] = useState<'edit-prompt' | 'processing'>('edit-prompt');
  ...
  useEffect(() => {
    if (isOpen) {
      setCustomPrompt(getActivePrompt());
  ...
  const processAll = useCallback(async () => {
    ...
    for (let i = 0; i < templates.length; i++) {
      ...
      // Pass the custom prompt as promptOverride
    ...
    fetch('/api/slack-notify', {
      ...
      body: JSON.stringify({ completed, failed, total: results.length }),
    }).catch(() => {});

    isProcessingRef.current = false;
  }, [templates, customPrompt]);

  // Start processing when phase changes to 'processing'
  useEffect(() => {
    if (phase === 'processing' && templates.length > 0) {
      processAll();
    }
  }, [phase, templates, processAll]);
  ...
  const handleClose = () => {
    if (phase === 'processing' && !state.isComplete) {
  ...
  // Step progress indicator (same as SimpleBatchModal)
          ...
          {phase === 'edit-prompt' ? (
            <>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Review the prompt for {templates.length} exercise(s). Edit if needed, or continue with the default:
              </p>

              <textarea
                value={customPrompt}
                onChange={(e) => setCustomPrompt(e.target.value)}
                className="w-full h-48 p-3 border border-gray-300 rounded-lg text-sm font-mono resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
