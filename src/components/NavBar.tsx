    ...
    <>
      <aside className={`flex flex-col h-screen bg-white border-r border-gray-200 flex-shrink-0 transition-all duration-200 ${collapsed ? 'w-16' : 'w-56'}`}>
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto overflow-x-hidden">
          ...
          {/* Image Gen — expandable tree */}
          <div>
            <button
              onClick={() => collapsed ? (window.location.href = '/image-gen') : setImageGenOpen(o => !o)}
              className={`w-full flex items-center gap-3 px-2.5 py-2 rounded-lg text-sm font-medium transition-colors ${
                pathname.startsWith('/image-gen') ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
              }`}
            >
              ...
              {!collapsed && (
                <>
                  <span className="flex-1 text-left">Image Gen</span>
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 flex-shrink-0 transition-transform duration-150 ${imageGenOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </>
              )}
            </button>
            {!collapsed && imageGenOpen && (
